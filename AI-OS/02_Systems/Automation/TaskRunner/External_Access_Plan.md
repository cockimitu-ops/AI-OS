...
**Correction, 2026-08-30 — tried twice, both blocked, nothing built.** OAuth was tried first and abandoned: registering a Google Cloud OAuth client requires a billing-enabled project. IMAP with a Gmail App Password was tried next, actually built and wired up, then abandoned too: App Passwords aren't available at all on Felix's account. Both attempts were fully removed rather than left disabled in the tree — see [[02_Systems/Automation/TaskRunner/README|TaskRunner's README]] for the detail. The risk analysis above (read vs. draft vs. send, the confirmation-gate requirement for anything past read-only) still holds regardless of mechanism — but the "concrete build shape" paragraph is wrong twice over now, and a third attempt needs an actual third mechanism, not another pass at either of these two.
