...
| `scripts/flip_log.py` | LocalArbitrage flip logging + ROI/EUR-per-hour (added 2026-08-31). Reads and writes `10_Projects/LocalArbitrage/Transaction_Log.md`'s own Markdown table directly - no separate ledger to drift from it. `buy`/`sell`/`report` subcommands. See that project's own doc for the design reasoning. |
| `scripts/study_agent.py` | Study note ingestion (added 2026-08-31) — reads raw notes from `10_Projects/CyberSecurityLearning/Inbox/`, hands each to the Study Teacher agent through the normal task queue, and files the processed result (summary, concepts, action items, flashcards) into the vault via `vault_write.py`, logged in that project's `Study_Log.md`. The model returns text only — discovery, dedupe-by-content-hash, destination, headers and logging are all deterministic here. Source notes are never modified. `aios-study.timer` runs it nightly at 21:30. |
...
**Deliberately not filed under `04_Agents/`.** `04_Agents/` holds *persona definitions* (role, scope, executable prompt); this folder holds the *execution infrastructure* that runs them. Since 2026-08-30 the two are wired together — the worker loads a persona per task — but they remain separate concerns: one describes who acts, the other is the machinery that makes it act unattended. Filing the machinery under the persona definitions would conflate the two.
