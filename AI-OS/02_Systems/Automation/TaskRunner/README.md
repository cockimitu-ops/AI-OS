...
| `scripts/mail_read.py` | Read-only Gmail via IMAP + App Password (added 2026-08-30) — see below. |
...
**Gmail read access — built 2026-08-30, via IMAP, not OAuth.** The first approach tried was a Google Cloud OAuth client (device flow, `gmail.readonly`, `scripts/gmail_oauth_setup.py`) — abandoned the same day, before finishing, because registering the OAuth client required a billing-enabled Google Cloud project. `scripts/mail_read.py` replaces it: plain IMAP (`imaplib`, stdlib) against `imap.gmail.com` using a Gmail **App Password**, which needs 2-Step Verification on the account and a password generated at `myaccount.google.com/apppasswords` — no Cloud project, no billing, no OAuth consent screen at all. Every fetch uses `BODY.PEEK[...]`, never plain `BODY`, so reading a message for the summary never marks it as read on the actual account — a read-only tool that has the side effect of changing your inbox would defeat the point.

Setup is two lines Felix adds to `.env` directly — `GMAIL_ADDRESS` and `GMAIL_APP_PASSWORD` — never pasted through Claude, same reasoning as everything else touching credentials here. `format_email_section()` in `morning_brief.py` calls `mail_read.fetch_unread_summaries()`, which returns `None` (not an empty list) until both are set, so the digest keeps showing "still waiting" instead of silently going quiet.
