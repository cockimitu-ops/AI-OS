...
- **OpenRouter: unexercised.** It sits last in the chain and the one live test so far succeeded on an earlier entry before reaching it, so it hasn't actually been proven to work yet — only confirmed the model ID is currently live on OpenRouter's own model list, which is a different claim than "this key can call it successfully."
